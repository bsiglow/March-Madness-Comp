...
OG_RegSeasonStat <-  read.csv("https://www.dropbox.com/s/n28iujsk23aqyq9/MRegularSeasonDetailedResults.csv?dl=1")
...
rank_table <-  read.csv("https://www.dropbox.com/s/hf7omb5pgmv55zb/MMasseyOrdinals.csv?dl=1")
...
write.csv(Teams_rank, file = "Kaggle_Team_Table")
```
...
<!-- Team_table_csv <-  load("Kaggle_Team_Table") # This will be TeamsTable made above. -->
...
# Thi
Team_Table_NET <- merge(Teams_rank, cc1, by.x = "TeamID_Year", by.y = "V1", all.x = TRUE) # merginig to create table with final NET score

# This needs changed when the data load issue is fixed

Team_Table_NET <- Team_Table_NET[,-c(23, 24, 25,26)] # Removing uneeded columns
...
```{r}
save(Team_Table_NET_AP, file = "Team_Season_Kaggle_2021") # Saving the final team Season Data
...
# When everything is resaved use these loads
...
```{r}
colnames(Team_Table_NET_AP)[20] <- "X2pt_3pt_ratio"
```
...
save(Game_full, file = "Kaggle_game_stats")
...
xgb.save(Gamebeast, "Kaggle1_xgmodel") ## This can also save an rda 
```
...
```{r}
Tourney_seed <-  read.csv("https://www.dropbox.com/s/v7l5m7sf4uivf3f/MNCAATourneySeeds%20%281%29.csv?dl=1")
load("Team_Season_Kaggle_2021")

Tourney_seed$TeamID_Year <-  paste(Tourney_seed$Season, Tourney_seed$TeamID)

Tourney_seed <- Tourney_seed[Tourney_seed$Season==2021,-3]
```
...
W16_dat <- xgb.DMatrix(data = as.matrix(W16[, names(train_strat)[c(3:24, 27:47)]]))
...
X11a <- Team_Table_NET_AP[Team_Table_NET_AP$TeamID_Year == Tourney_seed$TeamID_Year[Tourney_seed$Seed == "X11a"],]

names(X11a) <- paste("R", names(X11a), sep = "")
  
X11b <- Team_Table_NET_AP[Team_Table_NET_AP$TeamID_Year == Tourney_seed$TeamID_Year[Tourney_seed$Seed == "X11b"],]

names(X11b) <- paste("O", names(X11b), sep = "")


X11 <-  cbind(X11a, X11b)

X11_dat <- xgb.DMatrix(data = as.matrix(X11[, names(test_strat)[c(3:24, 27:47)]]))
X11_pred <-  predict(Gamebeast, X11_dat)
X11_pred > .5
# X11a is predicted to win

#X16 seed prediction. 
 
X16a <- Team_Table_NET_AP[Team_Table_NET_AP$TeamID_Year == Tourney_seed$TeamID_Year[Tourney_seed$Seed == "X16a"],]

names(X16a) <- paste("R", names(X16a), sep = "")
  
X16b <- Team_Table_NET_AP[Team_Table_NET_AP$TeamID_Year == Tourney_seed$TeamID_Year[Tourney_seed$Seed == "X16b"],]

names(X16b) <- paste("O", names(X16b), sep = "")


X16 <-  cbind(X16a, X16b)

X16_dat <- xgb.DMatrix(data = as.matrix(X16[, names(test_strat)[c(3:24, 27:47)]]))
X16_pred <-  predict(Gamebeast, X16_dat)
X16_pred > .5
# X16a is predicted winner

```

# Stop here
```{r :: Create the tournament for 2021}
# Change Values to most recent data and 2021

Tourney <- read.csv("https://www.dropbox.com/s/zhb810pe94bd8ik/Tourney_base_table?dl=1")

Tourney_seed_2021 <-  Tourney_seed[!(Tourney_seed$Seed == 'W11a' | Tourney_seed$Seed == 'W16b' | Tourney_seed$Seed == "X11b" | Tourney_seed$Seed == "X16b"),]

Tourney_seed_2021$Seed <- gsub("a", "", Tourney_seed_2021$Seed)
Tourney_seed_2021$Seed <- gsub("b", "", Tourney_seed_2021$Seed)


Tourney_seed1 <-  merge(Tourney_seed_2021, Tourney, by.x = "Seed", by.y = "Seed", all.y = TRUE)

Tourney_seed1 <-  Tourney_seed1[,-4]

Tourn_2021_64 <-  Tourney_seed1

save(Tourn_2021_64, file = "Kaggle_Tourney_2021_64")


```

# Load Tourn_base for future forecast.