...
```{r}
#rank_test_small <-  rank_test[1:500,]
#teams_all1_small <- teams_all1[1:500,]

#str(rank_test_small)

# check <-  rank_test_small %>%
  #filter(TeamID_Year == "2003 1328") %>%
  #select(OrdinalRank) %>%
  #as.data.frame()

#rank_test_small$TeamID_Year

#T_rank <- as.data.frame(rank_test_small$OrdinalRank[rank_test_small$RankingDayNum == max(rank_test_small$RankingDayNum)], rank_test_small$TeamID_Year)

#rank_test$final_rank <- rank_test$OrdinalRank[rank_test$RankingDayNum == max(rank_test$RankingDayNum)] 

Final_rank <- function(x){x[rank_test$RankingDayNum == max(rank_test$RankingDayNum)] ; x }  # Filling the value for final rank in effort to speed up loop

rank_test$Final_Rank <- Final_rank(rank_test$OrdinalRank) # Running function 

#head(rank_test$Final_Rank)
...
for (i in 1:nrow(teams_all1)){
  ...
  teams_all1[i,23] <- temp$Final_Rank[i]
