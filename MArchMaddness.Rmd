...
```{r :: Creating XGMatrix }
Gtrain <- xgb.DMatrix(data = as.matrix(train_strat[,c(3:24, 26:52)]), label = as.numeric(train_strat$Win) - 1)
# Create test matrix
Gtest <- xgb.DMatrix(data = as.matrix(test_strat[, c(3:24, 26:52)]), label = as.numeric(test_strat$Win) - 1)
